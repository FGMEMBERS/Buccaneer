...
# Set view eye paths.
	var view_cfg = "/sim/view[" ~ view.indexof("Observer View")~ "]/config";
...
setlistener("/sim/signals/fdm-initialized", func {
	setlistener("/sim/remote/connected", func {
		var connected = getprop("/sim/remote/connected");

		if (connected) {
			setprop("/sim/current-view/view-number", view.indexof("Observer View"));
			globals["observer-view"].model_view_handler.next(1);
		} else {
			setprop("/sim/current-view/view-number", view.indexof("Cockpit View"));
		}
